...
function Game.OnInitOK()
    ...
    CtrlManager.Init()
    -- 1. 主面板初始化
	local ctrl = CtrlManager.GetCtrl(CtrlNames.MainPanel)
    if ctrl ~= nil and AppConst.ExampleMode == 1 then
        ctrl:Awake()
    end
    -- 2. 产品列表初始化
	local ctrl = CtrlManager.GetCtrl(CtrlNames.ListPanel)
    if ctrl ~= nil and AppConst.ExampleMode == 1 then
        ctrl:Awake()
    end
	-- 3. 搜索面板初始化
	local ctrl = CtrlManager.GetCtrl(CtrlNames.SearchPanel)
    if ctrl ~= nil and AppConst.ExampleMode == 1 then
        ctrl:Awake()
    end
	
	-- 4. 忘记密码面板初始化
	local ctrl = CtrlManager.GetCtrl(CtrlNames.FetchPwdPanel)
    if ctrl ~= nil and AppConst.ExampleMode == 1 then
        ctrl:Awake()
    end
		
	-- 5. 注册面板初始化
	local ctrl = CtrlManager.GetCtrl(CtrlNames.SignUpPanel)
    if ctrl ~= nil and AppConst.ExampleMode == 1 then
        ctrl:Awake()
    end
	
	-- 6. 登录面板初始化
	local ctrl = CtrlManager.GetCtrl(CtrlNames.LoginPanel)
    if ctrl ~= nil and AppConst.ExampleMode == 1 then
        ctrl:Awake()
    end
